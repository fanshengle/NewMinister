//
//  NMSecondSearchViewController.m
//  NewMinister
//
...
#import "NMGoodsListViewController.h"
#import "NMUsersListViewController.h"

@interface NMSecondSearchViewController ()<UIScrollViewDelegate,SPPageMenuDelegate>
{
...
@property (nonatomic,strong) UIView *generalTopView;                                   //共有topView
...
@implementation NMSecondSearchViewController
...
- (void)viewDidLoad {
    ...
    [self.generalTopView addSubview:self.topSearchView];
    [self.generalTopView addSubview:self.pageMenuControllView];
    [self setupPageMenuController];
}

#pragma mark ====================创建视图区=====================
#pragma mark -- 设置随手记、商品、用户三者共有的topView
- (UIView *)generalTopView{
    if (!_generalTopView) {
        
        UIView *generalTopView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, NMScreenWidth, NM88)];
        generalTopView.backgroundColor = [UIColor whiteColor];
        [self.view addSubview:generalTopView];
        _generalTopView = generalTopView;
    }
    return _generalTopView;
}
...
- (NMTopOperationBarView *)topSearchView{
    if (!_topSearchView) {
        ...
        _topViewHeight = 56.;
        NMTopOperationBarView *topSearchView = [[NMTopOperationBarView alloc] initTopSearchViewHaveLeftBtnWithFrame:CGRectMake(0, 0, NMScreenWidth, _topViewHeight) rightTitle:@"取消" rightImgStr:nil];
...
- (SPPageMenu *)pageMenuControllView{
    ...
    if (!_pageMenuControllView) {
        ...
        self.pageMenuTitleArr = @[@"随手记",@"商品",@"用户"];
        CGFloat menuViewLeft = 100.;
        CGFloat menuViewWidth = NMScreenWidth - menuViewLeft*2;
        _menuViewHeight = 32.;
        //trackerStyle:跟踪器的样式
        SPPageMenu *pageMenuControllView = [SPPageMenu pageMenuWithFrame:CGRectMake(menuViewLeft, self.topSearchView.height, menuViewWidth, _menuViewHeight) trackerStyle:SPPageMenuTrackerStyleLine];
        
        pageMenuControllView.selectedItemIndex  = self.currentMenuSelectIndex;
        //隐藏分割线
        ...
        // 不可滑动的自适应内容排列
        pageMenuControllView.permutationWay = SPPageMenuPermutationWayNotScrollAdaptContent;
        //传递数组，默认选中第0个
        [pageMenuControllView setItems:self.pageMenuTitleArr selectedItemIndex:_currentMenuSelectIndex];
...
- (UIScrollView *)pageMenuScrollView {
    ...
    if (!_pageMenuScrollView) {
        ...
        _scrollViewHeight = NMScreenHeight - _topViewHeight - _menuViewHeight;
...
- (void)setupPageMenuController{
    ...
    self.childControllersArr = [NSMutableArray array];
     NSArray *controllerClassNames = [NSArray arrayWithObjects:@"NMNotesListViewController",@"NMGoodsListViewController",@"NMUsersListViewController",nil];
    for (int i = 0; i < self.pageMenuTitleArr.count; i++) {
        
        if (controllerClassNames.count > i) {
            
            NMBaseViewController *baseVC = [[NSClassFromString(controllerClassNames[i]) alloc] init];
